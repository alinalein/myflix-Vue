...
export const getStoredUser = (): { Username: string; Email: string; Birthday: string; FavoriteMovies: string[] } | null => {
    ...
    try {
        return JSON.parse(rawUser) as { Username: string; Email: string; Birthday: string; FavoriteMovies: string[] };
    }
    catch (error) {
...
export const addMovie = async (id: string) => {
    try {
        ...
        const response = await axios.put<AddMovieResponse>(`https://movie-api-lina-834bc70d6952.herokuapp.com/users/${user.Username}/movies/add/${id}`,
            {},
            {
...
export const deleteMovie = async (id: string) => {
    try {
        ...
        const response = await axios.delete<DeleteMovieResponse>(`https://movie-api-lina-834bc70d6952.herokuapp.com/users/${user.Username}/movies/remove/${id}`,
            {
                headers: { Authorization: `Bearer ${token}` }
            })
        if (response.status === 200) {
...
export const fetchMovies = async (): Promise<Movie[] | undefined> => {
    try {
        ...
        if (!userToken) return
        const response = await axios.get<Movie[]>('https://movie-api-lina-834bc70d6952.herokuapp.com/movies', {
